...
class YouTubeTranscriptGUI:
    ...
    def setup_ui(self):
        ...
        title = ttk.Label(header, text="YouTube Transcript Downloader",
                          style='Header.TLabel')
        subtitle = ttk.Label(
            header, text="Minimal, clean and fast", style='HeaderSub.TLabel')
        ...
        # URL input
        ttk.Label(card, text="YouTube URL", style='Label.TLabel').grid(
            row=0, column=0, sticky=tk.W, pady=(0, 8))
        self.url_entry = ttk.Entry(
            card, textvariable=self.url_var, width=50, style='Input.TEntry')
        self.url_entry.grid(row=0, column=1, sticky=(
            tk.W, tk.E), pady=(0, 8), padx=(12, 0))

        # Output filename
        ttk.Label(card, text="Filename", style='Label.TLabel').grid(
            row=1, column=0, sticky=tk.W, pady=(0, 8))
        self.output_entry = ttk.Entry(
            card, textvariable=self.output_var, width=50, style='Input.TEntry')
        self.output_entry.grid(row=1, column=1, sticky=(
            tk.W, tk.E), pady=(0, 8), padx=(12, 0))

        # Buttons
        buttons_frame = ttk.Frame(card, style='Card.TFrame')
        buttons_frame.grid(row=2, column=0, columnspan=2,
                           pady=18, sticky=(tk.W, tk.E))
        for c in range(3):
        ...
        self.download_button.grid(
            row=0, column=1, padx=6, sticky=(tk.W, tk.E))
        ...
        self.progress = ttk.Progressbar(
            card, mode='indeterminate', style='Thin.Horizontal.TProgressbar')
        self.progress.grid(row=3, column=0, columnspan=2,
                           sticky=(tk.W, tk.E), pady=(0, 6))
        self.progress.grid_remove()

        # Preview label
        preview_label = ttk.Label(
            card, text="Preview", style='Section.TLabel')
        preview_label.grid(row=4, column=0, columnspan=2,
                           sticky=tk.W, pady=(8, 6))

        # Preview area
        preview_frame = ttk.Frame(card, style='CardInner.TFrame')
        preview_frame.grid(row=5, column=0, columnspan=2,
        ...
        self.preview_text.grid(row=0, column=0, sticky=(
            tk.W, tk.E, tk.N, tk.S))
        self.preview_text.configure(font=self.font_mono, background=self.colors['editor_bg'],
        ...
        self.status_label = ttk.Label(
            status_frame, textvariable=self.status_var, anchor=tk.W, style='Status.TLabel')
    ...
    def download_transcript(self):
        ...
        if save_path:
            try:
                ...
                messagebox.showinfo(
                    "Success", f"Transcript successfully saved to {save_path}")
    ...
    def _fetch_worker(self):
        try:
            ...
            self.root.after(
                0, self._fetch_error, f"An unexpected error occurred: {e}")
    ...
    def _setup_style(self):
        ...
        try:
            tkfont.nametofont('TkDefaultFont').configure(
                family=self.font_base[0], size=self.font_base[1])
            tkfont.nametofont('TkTextFont').configure(
                family=self.font_base[0], size=self.font_base[1])
            tkfont.nametofont('TkHeadingFont').configure(
                family=self.font_base[0], size=self.font_base[1] + 2)
            tkfont.nametofont('TkFixedFont').configure(
                family=self.font_mono[0], size=self.font_mono[1])
        except Exception:
            pass
        ...
        style.configure('TLabel', background=self.colors['card'],
                        foreground=self.colors['text'], font=self.font_base)
        style.configure('Label.TLabel', background=self.colors['card'],
                        foreground=self.colors['muted'], font=self.font_base)
        ...
        style.configure('Header.TLabel', background=self.colors['header_bg'],
                        foreground=self.colors['header_text'], font=self.font_title)
        style.configure('HeaderSub.TLabel', background=self.colors['header_bg'], foreground='#D1D5DB',
        ...
        style.configure('Accent.TButton', font=self.font_base,
                        padding=8, foreground='white', background=self.colors['primary'])
        ...
        style.configure('Status.TLabel', background=self.colors['bg'],
                        foreground=self.colors['muted'], font=self.font_status)
